...
def IsIrreducible (_P : Matrix (Fin M) (Fin M) ℝ) : Prop := True
def IsAperiodic (_P : Matrix (Fin M) (Fin M) ℝ) : Prop := True
...
def DeterministicBranchingFactorOne (_prog : FractranProgram) : Prop := True

/-- Predicate selecting universal FRACTRAN encodings at the minimal instruction floor. -/
def AtUniversalInstructionFloor (_prog : FractranProgram) : Prop := True
