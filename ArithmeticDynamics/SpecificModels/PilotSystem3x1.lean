...
axiom collatz_div_cond : ∀ (i : Fin 2) (k : ℤ),
  (2 : ℤ) ∣ ((if i.val = 0 then 1 else 3) * (k * 2 + i.val) + (if i.val = 0 then 0 else 1))
...
axiom collatz_drift_is_contractive :
  ErgodicTheory.logarithmicDrift 2 (fun i => if i.val = 0 then 1 else 3) < 0
