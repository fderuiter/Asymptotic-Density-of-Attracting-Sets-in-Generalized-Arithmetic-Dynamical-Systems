...
/-- The strict sub-language ordering on the Chomsky hierarchy,
generated by Type3 ≤ Type2 ≤ Type1 ≤ Type0 with reflexivity and transitivity. -/
...
structure MealyMachine (Sigma : Type) where
  /-- The (possibly infinite) state space of the transducer. -/
  State : Type
  /-- The state-transition function: given a state and input symbol, produce the next state. -/
  transition : State → Sigma → State
  /-- The output function: given a state and input symbol, produce the output symbol. -/
  output : State → Sigma → Sigma

-- To define ObservationalEquivalence without it failing, let's opaque it
/-- Observational equivalence between a `d`-adic map and a Mealy Machine transducer:
they produce identical output sequences on every input. -/
...
/-- The Chomsky-hierarchy level attained by the computational capacity of a `d`-adic map `f`. -/
