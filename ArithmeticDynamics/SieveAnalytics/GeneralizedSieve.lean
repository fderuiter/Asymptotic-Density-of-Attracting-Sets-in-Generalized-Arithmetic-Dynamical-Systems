...
theorem main_term_extraction :
  ∃ (ε : ℝ), ε > 0 ∧ ε < 1 ∧
  ∀ (X : ℝ), ∃ (O : ℝ), O > 0 := by sorry -- placeholder for O(X^{1 - ε}) main term bounding
