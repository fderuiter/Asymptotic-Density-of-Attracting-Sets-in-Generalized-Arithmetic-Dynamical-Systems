...
/-- Abstract type representing the phase space equipped with the primal topology τ_f. -/
...
/-- The canonical map whose primal topology τ_f is analyzed. -/
noncomputable opaque f : StateSpace → StateSpace
/-- The primal topology τ_f on `StateSpace`, generated by the forward-invariant open sets of `f`. -/
noncomputable opaque tau_f : TopologicalSpace StateSpace
/-- The metric entropy of `f_map` relative to measure `μ` in the primal topology τ_f. -/
...
/-- Proposition asserting that the set of periodic orbits of `f` is finite. -/
opaque periodic_orbits_finite : Prop
/-- Proposition asserting that every bounded continuous potential has an equilibrium state. -/
